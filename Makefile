...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=gerta \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=gertad \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/gertad ./cmd/gertad
endif

build-linux:
	mkdir -p $(BUILDDIR)
	docker build --no-cache --tag gertamoney/core ./
	docker create --name temp gertamoney/core:latest
	docker cp temp:/usr/local/bin/gertad $(BUILDDIR)/
	docker rm temp

build-linux-with-shared-library:
	mkdir -p $(BUILDDIR)
	docker build --tag gertamoney/core-shared ./ -f ./shared.Dockerfile
	docker create --name temp gertamoney/core-shared:latest
	docker cp temp:/usr/local/bin/gertad $(BUILDDIR)/
	docker cp temp:/lib/libwasmvm.so $(BUILDDIR)/
	docker rm temp

install: go.sum 
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/gertad
...
draw-deps:
	...
	@goviz -i ./cmd/gertad -d 2 | dot -Tpng -o dependency-graph.png
