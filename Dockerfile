...
# See https://github.com/gerta-chain/wasmvm/releases
...
RUN addgroup gerta \
    && adduser -G gerta -D -h /gerta gerta

WORKDIR /gerta

COPY --from=go-builder /code/build/gertad /usr/local/bin/gertad
...
CMD ["/usr/local/bin/gertad", "version"]
